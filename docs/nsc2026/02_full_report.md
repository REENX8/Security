...
| ความยั่งยืนของโค้ด | Test coverage + CI gate | ✅ 242 tests, recall ≥ 0.85 gate |
...
| Testing | pytest 8.3, httpx 0.28 (TestClient) — 242 cases |
...
phishing จึงต่ำกว่าและ**ผันผวนสูงตาม snapshot ของ feed** — เป็น cross-check ไม่ใช่ metric หลัก
และไม่ผูกค่าตายตัวในเอกสาร วัดสำหรับโมเดลปัจจุบันด้วย `make evaluate` (ต้องเข้าถึง OpenPhish)
