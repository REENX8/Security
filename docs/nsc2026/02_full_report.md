...
ผลการทดสอบบนชุดข้อมูล Thai-targeting holdout ที่โมเดลไม่เคยเห็น (378 URL)
ได้ recall **100% (378/378)** ที่ threshold ≥ 0.7
...
On the Thai-targeting holdout (378 URLs the model never sees during
training), the system achieves **100% recall (378/378)** at the
...
1. **วัตถุประสงค์หลัก** — สร้างระบบตรวจจับ URL ฟิชชิงเฉพาะทางที่จับการปลอม
   ...
   โมเดลไม่เคยเห็น (ปัจจุบันได้ **100% (378/378)** บน Thai-targeting holdout)
2. **วัตถุประสงค์รอง**
...
| ความยั่งยืนของโค้ด | Test coverage + CI gate | ✅ 258 tests, recall ≥ 0.85 gate |
...
| Testing | pytest 8.3, httpx 0.28 (TestClient) — 258 cases |
...
| Mean score | 0.986 | 0.994 | **0.999** |

> v1.5.0 เพิ่ม 5 features (TLS-derived + interaction) ทำให้ URL ที่เคยหลุดใน v1.3.0
> (`thaid-app.net/auth/login`) ถูกจับได้ → recall ขึ้นเป็น 100%

### Generic Phishing Holdout (Cross-check — ทางเลือก)

ระบบจูนให้เน้น Thai-targeting โดยตั้งใจ (alignment score เป็นบวก) recall บน generic
phishing จึงต่ำกว่า Thai holdout — เป็น cross-check ไม่ใช่ metric หลัก เราใช้ committed
snapshot ของ OpenPhish/URLhaus (`data/generic_phishing_seed.csv`, 300 URL non-Thai) split
70/30 ด้วย seed คงที่ → holdout 90 URL วัด recall ได้ **91.11% (82/90)** ที่ threshold ≥ 0.7
reproduce ได้ offline ด้วย `make evaluate` ตัวเลขนี้จึงคงที่ ไม่ผันผวนตาม feed อีกต่อไป
**หมายเหตุ:** host ~24% ซ้ำกันระหว่าง train/holdout ของ generic จึงเป็น in-distribution
cross-check ไม่ใช่การทดสอบ phishing แปลกใหม่ที่เป็นอิสระจริง
...
| Synthetic data → recall ดีจอมปลอม | แยก holdout จริง 378 URL ออกก่อน train, ใช้เป็น primary metric |
