...
[![Tests](https://img.shields.io/badge/tests-<!--M:test_count-->549<!--/M-->%20passing-success)](tests/)
...
**Feature Schema v<!--M:schema_version-->1.9.0<!--/M--> — <!--M:n_features-->47<!--/M--> features** — สัญญา feature-matrix ล่าสุด:
...
### 🎯 Primary — Thai-targeting phishing holdout (schema v<!--M:schema_version-->1.9.0<!--/M-->)
...
**<!--M:generic_recall-->94.44% (85/90)<!--/M-->** โดย **Thai recall ยังคง 100% (378/378)**
...
ตัวเลขนี้สูงเป็นปกติเพราะ train/test มาจาก SyntheticGenerator ตัวเดียวกัน — ใช้เป็น sanity check ภายใน (training rows 12,000 rows)
...
├── scripts/
│   ├── seed_demo.py                      #  smoke-test golden URLs against live API
│   ├── expand_whitelist.py               #  pull new .go.th / .ac.th / .or.th domains
│   ├── collect_thai_phishing_seed.py     #  grow Thai-targeting seed corpus
│   ├── collect_generic_phishing_seed.py  #  fetch OpenPhish/URLhaus snapshot (non-Thai)
│   ├── audit_seed_coverage.py            #  make seed-audit — brand × pattern coverage
│   ├── sync_docs_metrics.py              #  make sync-docs — inject eval metrics into docs
│   ├── build_extension.py                #  make extension / --check (CI)
│   ├── demo_setup.sh / demo_reset.sh     #  spin up / tear down demo environment
│   └── demo_verify.py                    #  end-to-end golden-URL smoke test
├── Makefile                          #  make install test lint run train evaluate ...
├── docker-compose.yml
...
| Domain    | domain_age_days, is_thai_tld, tld_type_enc, is_known_registrar |
...
ซอฟต์แวร์นี้เป็นผลงานที่พัฒนาขึ้นภายใต้โครงการ "ระบบตรวจจับเว็บไซต์ฟิชชิงสำหรับ
หน่วยงานราชการและสถาบันการศึกษาไทย ด้วยการเรียนรู้ของเครื่องและกฎอ้างอิงที่
โปร่งใส" ซึ่งสนับสนุนโดย **สำนักงานพัฒนาวิทยาศาสตร์และเทคโนโลยีแห่งชาติ (สวทช.)**
ในการแข่งขันพัฒนาโปรแกรมคอมพิวเตอร์แห่งประเทศไทย ครั้งที่ 28 (**NSC 2026**)
โดยมีวัตถุประสงค์เพื่อส่งเสริมให้นักเรียนและนักศึกษาได้เรียนรู้และฝึกทักษะใน
การพัฒนาซอฟต์แวร์

ลิขสิทธิ์ของซอฟต์แวร์นี้เป็นของผู้พัฒนา ซึ่งได้อนุญาตให้ สวทช. เผยแพร่ซอฟต์แวร์
นี้ตาม "ต้นฉบับ" โดยไม่มีการแก้ไขดัดแปลงใด ๆ ทั้งสิ้น ให้แก่บุคคลทั่วไปได้ใช้
เพื่อประโยชน์ส่วนบุคคลหรือประโยชน์ทางการศึกษาที่ไม่มีวัตถุประสงค์ในเชิงพาณิชย์
โดยไม่คิดค่าตอบแทนการใช้ซอฟต์แวร์ สวทช. ไม่มีหน้าที่ในการดูแล บำรุงรักษา
จัดการอบรมการใช้งาน หรือพัฒนาประสิทธิภาพซอฟต์แวร์ รวมทั้งไม่รับรองความ
ถูกต้องหรือประสิทธิภาพการทำงานของซอฟต์แวร์ ตลอดจนไม่รับประกันความเสียหาย
ต่าง ๆ อันเกิดจากการใช้ซอฟต์แวร์นี้ทั้งสิ้น

ดูฉบับเต็มทั้งภาษาไทยและภาษาอังกฤษได้ที่ [`docs/nsc2026/06_disclaimer.txt`](docs/nsc2026/06_disclaimer.txt)
หรือเรียกผ่าน API ที่ `GET /api/v1/disclaimer`
...
| ตำแหน่ง | ข้อมูล |
|---------|--------|
| **ผู้พัฒนาหลัก** | [REENX8](https://github.com/REENX8) |
| **อีเมล** | asdawesdzd22@gmail.com |
