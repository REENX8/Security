...
```
                  ...
                  Browser ext   Dashboard   API client   Public feed
                          ...
                          ┌─────────▼──────────┐
                          │ scikit-learn + XGB │
                          │ ensemble (.pkl)    │  schema v1.4.0 · 37 features
                          └────────────────────┘
```
...
- `query_param_count` (จำนวน query parameters — redirect kits มีหลาย param)
- `path_entropy` (Shannon entropy ของ path — random path มี entropy สูง)
- `host_token_count` (จำนวน token ใน hostname — brand-stuffed domains มีมาก)
...
|-----------------------------|--------|
| Sample size                  | 90 URLs |
| Recall ที่ threshold ≥ 0.7   | **98.9% (89/90)** |
| 95% CI                       | [0.94, 1.00] |
| คะแนนเฉลี่ย                  | 0.985 |
...
ตัวเลขนี้สูงเป็นปกติเพราะ train/test มาจาก SyntheticGenerator ตัวเดียวกัน — ใช้เป็น sanity check ภายใน (training rows เพิ่มจาก 6,000 → 12,000 ใน v1.4.0)
...
│   ├── schema.py             #    37 features + LOGIN_KEYWORDS + SUSPICIOUS_TLDS (v1.4.0)
...
### Feature ทั้ง 37 ตัว (schema v1.4.0)
